 ...
 */
    ...
    (() => {
        ...
        const navigationSlider = (prevEl, nextEl) => ({
            prevEl,
            nextEl
        });
        const autoPlaySlider = (delay, disable) => ({
            delay,
        ...
        const initTabSlider = (sliderArray, matchMedia) => {
            sliderArray.forEach((slider => {
                if (matchMedia && !matchMedia.matches) {
                    ...
                    if (objectModules.tabSlider && objectModules.tabSlider.enabled) objectModules.tabSlider.destroy(true, true);
                } else {
        ...
        const initSliders = () => {
            ...
            if (heroSlider) {
                ...
                new Swiper(heroSlider, {
                    ...optionsSlider(heroSlider, Navigation, Pagination, Autoplay),
                    speed: speedSlider,
                    loop: true,
                    on: {
                        init: slider => {
            ...
            if (testimonialsSlider) {
                new Swiper(testimonialsSlider, {
                    ...optionsSlider(testimonialsSlider, Navigation, Pagination),
                    autoHeight: true,
                    on: {}
                });
            }
            if (workSlider) {
                const mainSlider = workSlider.querySelector("[data-work-slider-main]");
                const navSlider = workSlider.querySelector("[data-work-slider-nav]");
                const workSliderNav = new Swiper(navSlider, {
                    ...optionsSlider(navSlider),
                    spaceBetween: 10,
                    slidesPerView: 4.2,
                    watchSlidesProgress: true,
                    breakpoints: {
                ...
                new Swiper(mainSlider, {
                    ...optionsSlider(mainSlider, Navigation, Thumb),
                    spaceBetween: 20,
                    thumbs: {
            ...
            if (historySlider) {
                const historySwiper = new Swiper(historySlider, {
                    ...optionsSlider(historySlider, Navigation),
                    spaceBetween: 10,
                    autoHeight: true,
                    navigation: {
                        prevEl: ".history-top__prev",
                        nextEl: ".history-top__next"
                    },
                    on: {}
                });
                ...
                const historyAction = e => {
                    ...
                    historySwiper.slideTo(activeIndexButton);
                };
                historyNav.addEventListener("click", historyAction);
            }
            if (teamSlider) {
                const teamSliderArr = document.querySelectorAll("[data-slider]");
        ...
        const headerHeight = () => {
            ...
            const debouncedGetHeaderHeight = debounce(getHeaderHeight, 200);
            window.addEventListener("orientationchange", getHeaderHeight);
            window.addEventListener("resize", debouncedGetHeaderHeight);
            getHeaderHeight();
        };
        const menuStatus_menuClose = () => {
            bodyUnlock();
        ...
        const gotoBlock = function(targetSelector) {
            ...
            document.documentElement.classList.contains("menu-open") ? menuStatus_menuClose() : null;
        ...
        class FormsValidation {
            ...
            inputViewPass(target) {
                if (target.closest(`[class*="__${this.formClasses.viewPass}"]`)) {
                    let inputType = target.classList.contains(this.formClasses.viewPassActive) ? "password" : "text";
                    target.parentElement.querySelector("input").setAttribute("type", inputType);
                    target.classList.toggle(this.formClasses.viewPassActive);
                }
            }
            autoHeight() {
        ...
        class Modal {
            ...
            eventsModal() {
                document.addEventListener("click", (e => {
                    ...
                    if (buttonOpen) {
                        e.preventDefault();
                        this.dataValue = buttonOpen.getAttribute(this.options.attributes.openButton) || "error";
                        this.youTubeCode = buttonOpen.getAttribute(this.options.attributes.youtube) || null;
                        if (this.dataValue !== "error") {
                            ...
                            this.open();
                            return;
                        } else this.#modalLogging(`The y attribute is not filled ${buttonOpen.classList}`);
                        return;
                    }
                    if (buttonClose || !target.closest(`.${this.options.classes.modalContent}`) && this.#isOpen) {
                        e.preventDefault();
                        this.close();
                        return;
                    }
                ...
                if (bodyLockStatus) {
                    ...
                    if (this.targetOpen.element) {
                        if (this.youTubeCode) {
                            ...
                            const youtubePlace = this.targetOpen.element.querySelector(`[${this.options.attributes.youtubePlace}]`);
                        ...
                        if (!this.#reopen) !this.#bodyLock ? bodyScrollLock_bodyLock() : null; else this.#reopen = false;
                        ...
                        this.#modalLogging(`Відкрив модальне вікно`);
                    } else this.#modalLogging(`Modal not found. Check the selector.`);
                }
            }
            close(selectorValue) {
                ...
                this.#modalLogging(`Закрив модальне вікно`);
            }
            #getHash() {
                this.hash = this.targetOpen.selector.includes("#") ? this.targetOpen.selector : this.targetOpen.selector.replace(".", "#");
            }
            #openToHash() {
                const classInHash = document.querySelector(`.${window.location.hash.replace("#", "")}`) ? `.${window.location.hash.replace("#", "")}` : document.querySelector(`${window.location.hash}`) ? `${window.location.hash}` : null;
                const buttons = document.querySelector(`[${this.options.attributes.openButton} = "${classInHash}"]`) ? document.querySelector(`[${this.options.attributes.openButton} = "${classInHash}"]`) : document.querySelector(`[${this.options.attributes.openButton} = "${classInHash.replace(".", "#")}"]`);
                this.youTubeCode = buttons.getAttribute(this.options.attributes.youtube) || null;
        ...
        const circleProgress = () => {
            ...
            circleItems.forEach((circleItem => {
                ...
                if (circleItem.getAttribute("data-circle") === "percentage") {
                    const {percent} = circleItem.dataset;
                    const percentageProgress = Math.floor(percent);
                    setPregress(percent, percentageProgress);
                }
                if (circleItem.getAttribute("data-circle") === "fraction") {
                    const {current, total} = circleItem.dataset;
        ...
        const dataPortfolio = "resources/porfolio.json";
        const projectSlider = document.querySelector("[data-project-slider]");
        ...
        const displayPortfolio = portfolioArray => {
            if (projectSlider) {
                const typeSlider = projectSlider.dataset.projectSlider;
                const projectSliderWrapper = projectSlider.querySelector(".swiper-wrapper");
                projectSliderWrapper.insertAdjacentHTML("beforeend", projectHTML(portfolioArray, typeSlider));
                initProjectSlider();
                return;
            }
            if (portfolioContent) {
        ...
        const formatDateForTimeTag = dateStr => {
        ...
        const isYesterday = dateString => {
            ...
            if (givenDate.getFullYear() === yesterday.getFullYear() && givenDate.getMonth() === yesterday.getMonth() && givenDate.getDate() === yesterday.getDate()) return "1 day ago"; else return dateString;
        };
        ...
        const tabsNewsAction = e => {
            const {target} = e;
            if (target.closest(".tabs-news__button")) {
                const controlButton = target.closest(".tabs-news__button");
                if (controlButton.classList.contains("is-active")) return;
            ...
            if (objectModules.tabSlider && objectModules.tabSlider.enabled) {
                const sliderBullets = newsContent.querySelector("[data-slider-pagination]");
                !visibleItems ? sliderBullets.style.display = "none" : sliderBullets.style.display = "";
                objectModules.tabSlider.update();
                objectModules.tabSlider.slideTo(0);
            }
        };
        if (newsControls) newsControls.addEventListener("click", tabsNewsAction);
        const newsRender = async dataNews => {
            try {
                const response = await fetch(dataNews);
                if (!response.ok) throw new Error("Response is not OK");
        ...
        const displayPost = data => {
            ...
            const meta = pagePost.querySelectorAll(".meta-news");
            const post = postContent.querySelector(".post");
            const commentsContent = document.querySelector(".content-post__comments");
            const commentsTitle = commentsContent?.querySelector(".content-block__title");
            const commentsList = document.querySelector("[data-comments-list]");
            const {id, image, title, type, date, comments} = currentPost;
            post.id = id;
            postImage.src = image;
        ...
        const postRender = async dataNews => {
            try {
                const response = await fetch(dataNews);
                if (!response.ok) throw new Error("Response is not OK");
                const newsData = await response.json();
                displayPost(newsData.news);
            } catch (error) {
                postContent.innerHTML = `<div class="content-post__error">Failed to load post, please try again</div>`;
                const meta = document.querySelector(".meta-news");
                const comments = document.querySelector(".comments");
                const leaveComment = document.querySelector(".leave-comment");
                meta ? meta.style.display = "none" : null;
                comments ? comments.style.display = "none" : null;
        ...
        objectModules.modal = new Modal({
            on: {
                ...
                afterClose: modal => {
                    const form = modal.targetOpen.element.querySelector("form");
                    if (!form) return;
                    objectModules.formsValidation.formClean(form);
                }
            }
        });
    })();
